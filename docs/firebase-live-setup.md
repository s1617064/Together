...
- 登录后会先确认成员身份，再进入共享账本订阅。
- 只有看到“云同步已连接”这类提示后，才算真正连上账本。
...
- 账目按 `spentAt` 倒序实时监听
- 如果成员绑定、权限或网络出错，页面会直接显示明确错误，不再停留在“同步中”
...
1. 账号登录成功
2. 页面确认当前账号是不是共享账本成员
3. 如果是首次登录新设备，会先写入 `members/{auth.uid}`
4. 成员身份确认完成后，才开始订阅 `expenses`
5. 首次订阅成功后，页面才真正进入账本
