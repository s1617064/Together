...
const ledgerBreakdown = document.querySelector("#ledger-breakdown");
...
function renderLedger() {
  ...
  if (!filteredExpenses.length) {
    ledgerList.innerHTML = `<p class="empty-copy">这个筛选条件下还没有记录，换个条件看看。</p>`;
    return;
  }
      ...
      return `
        <article class="breakdown-row">
          ...
          <div class="breakdown-track">
            <span class="breakdown-fill" style="width: ${percentage}%"></span>
          </div>
          <p class="breakdown-note">${item.count} 笔记录</p>
        </article>
      `;
    })
    .join("");
...
function getLedgerListKicker() {
  ...
  const hasFilters =
    state.ledgerMemberFilter !== "all" || state.ledgerCategoryFilter !== "all";
